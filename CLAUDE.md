...
uni-app 采用文件系统式路由，路由配置在 `frontend/src/pages.json`，共 44 个路由条目。TabBar 使用 `custom: true` 自定义实现（`components/layout/CustomTabBar.vue`），5个入口：首页、资源、发布（中心按钮）、问答（`pages/question/index`）、我的。注意：`pages/community/index` 是独立的社区门户页（含社团/活动/问答聚合），通过首页入口跳转，不在 TabBar 内。H5 模式下有路由守卫（`router/index.ts`）。
...
- `components/mobile/` 和 `components/desktop/`：端适配组件（通过 `windowWidth > 768px` 判断）；`desktop/PCFloatingNav.vue` 为 PC 端悬浮快捷操作按钮
...
- `pages/home/components/hero/` 下存在 `_backup.vue` 和 `_new_styles.scss` 等备份文件，勿编辑，以当前活跃文件（无后缀 `_backup`）为准；`components/mobile/MobileHeader_backup.vue` 同理
