...
uni-app 采用文件系统式路由，路由配置在 `frontend/src/pages.json`，共 31 个页面。TabBar 使用 `custom: true` 自定义实现（`components/layout/CustomTabBar.vue`），5个入口：首页、资源、发布（中心按钮）、社区、我的。H5 模式下有路由守卫（`router/index.ts`）。
...
### API 层
- 前端 `services/` 下 15 个业务服务文件（含 `ai.ts` AI 助手服务），均调用 `utils/request.ts` 的统一请求类
