...
- ✅ **视觉层级**：Top 1-2 标签自动高亮（🔥⚡图标 + 渐变背景）
- ✅ **动态字号**：根据标签频次自动调整字体大小
- ✅ **展开/收起**：优雅的展开按钮，支持折叠超出数量的标签
- ✅ **数量显示**：可选显示标签计数（支持 k/w 格式化）
- ✅ **激活状态**：支持高亮当前选中的标签
- ✅ **丰富交互**：Hover 动效 + 按压反馈 + 阴影过渡
- ✅ **空状态**：无数据时显示友好的空状态提示
- ✅ **完全响应式**：自动适配容器宽度
...
| `highlightTop` | `boolean` | `true` | 是否高亮 Top 1-2 标签（🔥⚡图标 + 渐变背景） |
...
### 场景1：资源页右侧栏（Top高亮 + 动态字号 + 折叠）

适用于展示资源分类标签，Top 1-2 自动高亮，字号根据资源数量动态调整。
...
**效果**：
- 🔥 Top 1 标签：蓝色渐变背景 + 火焰图标
- ⚡ Top 2 标签：橙色渐变背景 + 闪电图标
- 显示标题栏 + 标签总数徽章
- 字号范围 22-32rpx，根据频次动态调整
- 默认显示前 10 个，超出可点击"展开更多 +4"
- 不显示每个标签的计数
